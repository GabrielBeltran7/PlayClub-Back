  ...
  const subadminUsername = username;
    ...
    for (const usuario of usuarios) {
      ...
      await Recargarpuntos.create({
        cantidad,
        usernameSubadmin:"SubAdmin",
        UserId: usuario.id,
        // Otros campos relacionados con la transacción
      });
    }

    return res.status(200).json({ message: `Se agregaron ${cantidad} puntos a ${usuarios.length} de tus usuarios.` });
...
// const agregarPuntosAUsuarios = async (req, res) => {
//   try {
//     let { cantidad, username } = req.body;
//     const subadminUsername = username.toLowerCase(); // Convierte a minúsculas
...
//     // Verificar si el usuario es un SubAdmin
//     if (subadminUsername === "subadmin") {
...
//     } else {
//       return res.status(400).json({ error: `No eres usuario autorizado para dar bonos.` });
//     }
//   } catch (error) {
//     return res.status(500).json({ error: error.message });
//   }
